...
abstract class StripeBase extends AbstractPaymentProvider<StripeOptions> {
  ...
  async initiatePayment({
    ...
    const intentRequest: Stripe.PaymentIntentCreateParams = {
      ...
      metadata: { medusa_session_id: data?.medusa_session_id as string },
      ...additionalParameters,
    }

    intentRequest.customer = context?.accountHolder?.id
  ...
  async createAccountHolder({
    ...
    const { accountHolder, customer } = context

    if (accountHolder?.id) {
      return { id: accountHolder.id as string }
    }
    ...
    const defaultAddress = customer.billing_address
  ...
  async deleteAccountHolder({
    ...
    try {
      await this.stripe_.customers.del(accountHolder.id)
      return {}
    } catch (e) {
  ...
  async listPaymentMethods({
    ...
    const customerId = context?.accountHolder?.id
    if (!customerId) {
      return []
    }
  ...
  async savePaymentMethod({
    ...
    const accountHolder = context?.accountHolder
